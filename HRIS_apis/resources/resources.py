...
class StaffLeaveRequestResource(Resource):
    ...
    def post(self):
        try:
            ...
            if leave_type_id == self.SICK_LEAVE_TYPE_ID:
                ...
                leave_days = (to_date - from_date).days + 1
                if leave_days > self.SICK_LEAVE_LIMIT:
    ...
    def check_leave(self, staff_id, leave_type_id):
        
        match leave_type_id:
            case 1:
                leaves = Salaries.query.filter(
                    Salaries.EmployeeId == staff_id
                )
                casualLeavesTotal = leaves.CasualLeaves
                remainingCasualLeaves = leaves.RemainingCasualLeaves
                
                return casualLeavesTotal, remainingCasualLeaves
...
class StaffLeaveRequestResource(Resource):
    ...
    def post(self):
        try:
            ...
            # Apply leave type-specific validations and checks
    ...
    def put(self, leave_request_id):
        try:
            ...
            leave_request = StaffLeaveRequest.query.filter_by(LeaveRequestId=leave_request_id).first()

            if not leave_request:
                return {"status": "error", "message": "Leave request not found"}, 404

            # Update only the fields that are present in the request data
            staff_id = leave_request_data.get('StaffId', leave_request.StaffId)
            from_date = leave_request_data.get('FromDate', leave_request.FromDate.strftime('%Y-%m-%d'))
            to_date = leave_request_data.get('ToDate', leave_request.ToDate.strftime('%Y-%m-%d'))
            leave_type_id = leave_request_data.get('LeaveTypeId', leave_request.LeaveTypeId)
            reason = leave_request_data.get('Reason', leave_request.Reason)
            leave_status_id = leave_request_data.get('LeaveStatusId', leave_request.LeaveStatusId)
            ...
            # Assuming validations pass, update the leave request
            leave_request.StaffId = staff_id
            leave_request.FromDate = from_date
            leave_request.ToDate = to_date
            leave_request.Reason = reason
            leave_request.LeaveStatusId = leave_status_id
            leave_request.Remarks = leave_request_data.get('Remarks', leave_request.Remarks)
            leave_request.ApprovedBy = leave_request_data.get('ApprovedBy', leave_request.ApprovedBy)
            leave_request.LeaveApplicationPath = leave_request_data.get('LeaveApplicationPath', leave_request.LeaveApplicationPath)
            leave_request.UpdaterId = leave_request_data.get('UpdaterId', leave_request.UpdaterId)
            leave_request.UpdateDate = datetime.utcnow()

            db.session.commit()

            return {"status": "success", "message": "Leave request updated successfully."}, 200
...
class EmployeeCreationResource(Resource):
    ...
    def put(self):
        try:
            logging.info(f"Received request to update employee record.")
                ...
                if table_name in allowed_tables:
                    ...
                    for record_fields in fields:
                        ...
                        if record_id:
                            ...
                            logging.info(f"Inserted new record into {table_name}")
